...
export default function EventList({ status }) {
    ...
    const fetchEvents = async () => {
      ...
      try {
        ...
        const formattedEvents = response.data.map((event) => ({
          ...
          mentorImage: event.created_by.photo.trim()==""? "https://img.freepik.com/free-vector/user-blue-gradient_78370-4692.jpg?semt=ais_hybrid":event.created_by.photo, // Default image as API doesn't provide one
  ...
  if (isLoading) return <div className='text-center text-slate-600'>Loading...</div>;
  if (error) return <div className='text-center text-slate-600'>Error: {error}</div>;
  ...
  return (
    <>
      ...
      {events.map((event, index) => (
        <Card key={index} className="p-6 bg-white rounded-3xl border shadow-md lg:max-w-md w-full hover:shadow-lg hover:scale-[1.01] transition-all cursor-pointer flex flex-col h-full">
